...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/21 06:10:31 by fech-cha          #+#    #+#             */
/*   Updated: 2021/12/22 20:37:56 by fech-cha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (0);
	reminder[fd] = ft_read_all(reminder[fd], fd);
	if (!reminder[fd])
		return (NULL);
	str = ft_getline(reminder[fd], &i);
	if (!str)
		return (0);
	reminder[fd] = ft_getrest(reminder[fd], &i);
	if (i == -1)
		return (ft_free(str, 0));
	return (str);
}
